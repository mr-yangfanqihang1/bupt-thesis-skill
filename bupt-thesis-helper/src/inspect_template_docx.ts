...
function parseStyles(stylesXml) {
  ...
  while ((match = styleRe.exec(stylesXml || '')) !== null) {
    ...
    if (/w:type="character"/i.test(head)) continue;
    if (/w:type="table"/i.test(head)) continue;
    if (/w:type="numbering"/i.test(head)) continue;
...
function styleSummary(name, style) {
  const parts = [];
  if (style.font.eastAsia) parts.push(style.font.eastAsia);
  if (style.font.ascii && style.font.ascii !== style.font.eastAsia) parts.push(style.font.ascii);
  if (style.sizePt) parts.push(`${style.sizePt}pt`);
  if (style.bold) parts.push('加粗');
  if (style.linePt) parts.push(`行距 ${style.linePt}pt`);
  if (style.firstLineCm) parts.push(`首行 ${style.firstLineCm}cm`);
  if (style.alignment) parts.push(`对齐 ${style.alignment}`);
