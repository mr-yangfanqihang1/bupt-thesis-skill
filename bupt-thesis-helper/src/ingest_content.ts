...
function guessHeadingPrefix(text, style) {
  ...
  if (!trimmed) return '';
  if (/^(摘要|ABSTRACT|目录|参考文献|致谢|附录)$/i.test(compact)) return '## ';
  if (/^第[一二三四五六七八九十]+章\s+/.test(trimmed)) return '## ';
  if (/^\d+\.\d+\.\d+\s+/.test(trimmed)) return '#### ';
  if (/^\d+\.\d+\s+/.test(trimmed)) return '### ';
  if (/heading\s*1|title1|标题1|标题 1/i.test(style)) return '## ';
  if (/heading\s*2|title2|标题2|标题 2/i.test(style)) return '### ';
  if (/heading\s*3|title3|标题3|标题 3/i.test(style)) return '#### ';
  return '';
}
...
function isLikelyTocEntry(text, style) {
  ...
  if (isTocStyle(style)) return true;
  if (/[\.\u2026·]{2,}\s*\d+\s*$/.test(trimmed)) return true;
  if (/^(第[一二三四五六七八九十]+章|\d+(?:\.\d+){1,3})\s*.+\d+\s*$/.test(trimmed)) return true;
  return false;
}

function isBodyHeadingAfterToc(text, style) {
  const trimmed = String(text || '').trim();
  if (!trimmed || isLikelyTocEntry(trimmed, style)) return false;
  if (/^第[一二三四五六七八九十]+章\s+/.test(trimmed)) return true;
  if (/^(参考文献|致谢|附录)$/i.test(trimmed)) return true;
  if (/heading\s*1|title1|标题1|标题 1/i.test(style)) return true;
  return false;
}
...
async function ingestDocx(inputPath, options) {
  ...
  function flushPendingImageCaptions() {
    while (pendingImageCaptions.length) {
      ...
      if (pending.hasCaption) continue;
