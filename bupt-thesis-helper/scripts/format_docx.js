...
function injectOddPageSectionBeforeReferences(documentXml) {
    ...
    if (/w:val="oddPage"/i.test(before))
        return documentXml;
...
function requireGlobalLatinFont(defaults, hasStyleEntries) {
    ...
    if (!out) {
        throw new Error('thesis-format.json 缺少全局西文字体：请在 defaults 中设置 "latinFont": "Times New Roman" '
...
function relationTargetById(relsXml, id) {
    ...
    for (const block of blocks) {
        if (!new RegExp(`\\bId="${id}"`).test(block))
            continue;
...
function collectFooterPartPaths(documentXml, relsXml, applyTypes) {
    ...
    while ((m = re.exec(documentXml)) !== null) {
        ...
        if (target)
            out.add(footerTargetToPartPath(target));
    }
    return [...out];
}
...
function buildFooterParagraphXml(p) {
    ...
    const runs = (p.runs || []).map((r) => {
        if (r.kind === 'field')
            return footerFieldFragment(r.instruction, rpr);
...
function effectivePageFooter(pageFooter) {
    if (!pageFooter || typeof pageFooter !== 'object')
        return pageFooter;
...
async function patchFootersFromConfig(zip, documentXml, pageFooter) {
    ...
    if (!footerCfg || !Array.isArray(footerCfg.paragraphs) || !footerCfg.paragraphs.length)
        return;
...
function printFixPlan(documentXml, fixes, config, relsXml) {
    ...
    if (relsXml && config.pageFooter && wantsFix(fixes, 'page')) {
        footerTargets = collectFooterPartPaths(documentXml, relsXml, config.pageFooter.applyToFooterTypes).length;
    }
    const lines = [
        '可应用修复项:',
...
function styleParagraph(paragraphXml, style = {}, extra = {}) {
    ...
    if (line || resolvedBefore !== undefined || resolvedAfter !== undefined) {
        ...
        if (resolvedBefore !== undefined)
            attrs.push(`w:before="${Number(resolvedBefore) || 0}"`);
        if (resolvedAfter !== undefined)
            attrs.push(`w:after="${Number(resolvedAfter) || 0}"`);
        if (line)
            attrs.push(`w:line="${line}" w:lineRule="${lineRule}"`);
        pPrPatch['w:spacing'] = `<w:spacing ${attrs.join(' ')}/>`;
    }
    const indAttrs = [];
    if (!extra.indXml) {
        if (left !== null || extra.left !== undefined)
            indAttrs.push(`w:left="${extra.left !== undefined ? extra.left : left}"`);
        if (firstLine !== null || extra.firstLine !== undefined)
            indAttrs.push(`w:firstLine="${extra.firstLine !== undefined ? extra.firstLine : firstLine}"`);
        if (hanging !== null || extra.hanging !== undefined)
            indAttrs.push(`w:hanging="${extra.hanging !== undefined ? extra.hanging : hanging}"`);
    }
...
function styleRun(runXml, style = {}, forceBold = null) {
    ...
    if (/<w:rPr\b[\s\S]*?<\/w:rPr>/.test(runXml)) {
        let next = runXml.replace(/<w:rPr\b[\s\S]*?<\/w:rPr>/, (rPr) => {
            ...
            if (bold)
                patched = patched.replace(/<w:b\b[^>]*\/>/g, '').replace(/<w:bCs\b[^>]*\/>/g, '');
...
function isChapterHeading(text) {
    ...
    if (/^第(?:[一二三四五六七八九十百千]+|\d+)章[为是]/.test(trimmed))
        return false;
...
function textRunXml(text, options = {}) {
    ...
    if (options.font) {
        ...
        rPr.push(`<w:rFonts w:ascii="${escapeXml(f.ascii)}" w:eastAsia="${escapeXml(f.eastAsia)}" w:hAnsi="${escapeXml(f.hAnsi)}"/>`);
    }
...
function sectionHeadingDepthFromNormalizedText(norm) {
    const s = String(norm || '');
    if (/^\d+\.\d+\.\d+(?:\s+\S|(?=[\u4e00-\u9fffA-Za-z]))/.test(s))
        return 3;
    if (/^\d+\.\d+(?:\s+\S|(?=[\u4e00-\u9fffA-Za-z]))/.test(s))
        return 2;
    return 0;
}
...
function patchPageLayout(documentXml, config) {
    ...
    return documentXml.replace(/<w:sectPr\b[\s\S]*?<\/w:sectPr>/g, (sectPr) => {
        ...
        if (/<w:pgSz\b[^>]*\/>/.test(next))
            next = next.replace(/<w:pgSz\b[^>]*\/>/, pgSz);
        else
            next = next.replace('</w:sectPr>', `${pgSz}</w:sectPr>`);
        if (/<w:pgMar\b[^>]*\/>/.test(next))
            next = next.replace(/<w:pgMar\b[^>]*\/>/, pgMar);
        else
            next = next.replace('</w:sectPr>', `${pgMar}</w:sectPr>`);
        return next;
    });
}
async function formatDocx({ input, output, formatConfig, fixes, dryRun, keepComments, }) {
    ...
    if (!dryRun && output) {
        ...
        if (outResolved === inputPath) {
            throw new Error('输入与输出指向同一 DOCX，会覆盖源文件。请使用另一路径作为 --output（例如 原名-formatted.docx）。');
        }
    }
