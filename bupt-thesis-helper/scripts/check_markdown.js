...
function checkHeadingRules(blocks, issues) {
    ...
    for (const heading of headings) {
        if (prevLevel !== null && heading.level > prevLevel + 1) {
            addIssue(issues, 'error', 'heading.level_jump', heading.line, `标题层级跳跃：前一个标题级别为 H${prevLevel}，当前为 H${heading.level}。`, '避免从 `##` 直接跳到 `####`。');
        }
        ...
        if (heading.level === 2) {
            ...
            if (chapter !== null) {
                ...
                addIssue(issues, 'warning', 'heading.level1_format', heading.line, `一级标题 \`${heading.text}\` 不属于常见章标题格式。`, '若为正文一级标题，建议使用“第 X 章 ...”格式。');
            }
            continue;
        }
        if (heading.level === 3) {
        ...
        if (heading.level === 4) {
            ...
            if (!currentSection2) {
                ...
                if (chapterNo !== currentSection2[0] || sectionNo !== currentSection2[1]) {
                    addIssue(issues, 'error', 'heading.level3_parent_mismatch', heading.line, `三级标题编号为 ${chapterNo}.${sectionNo}.${subNo}，但当前二级标题为 ${currentSection2[0]}.${currentSection2[1]}。`);
                }
...
function checkAbstractRules(lines, blocks, issues) {
    [['摘要', KEYWORDS_ZH_RE, false], ['ABSTRACT', KEYWORDS_EN_RE, true]].forEach(([headingText, keywordRe, isEn]) => {
        ...
        if (!prevNonEmpty || (prevNonEmpty.text.trim() !== '<br>' && !prevNonEmpty.text.trim().endsWith('<br>'))) {
            addIssue(issues, 'error', 'abstract.br_before_keywords', keywordEntry.line, `\`${headingText}\` 的关键词前缺少 \`<br>\` 换行。`, '在摘要正文最后一行末尾追加 `<br>`，或单独写一行 `<br>` 后再写关键词。');
        }
    });
}
function recordCaption(issues, captionMap, captionType, lineNo, captionText, expectedChapter) {
    ...
    if (!match) {
        addIssue(issues, 'error', `caption.${captionType}_format`, lineNo, `${captionType === 'image' ? '图片' : '表格'}题注格式不合法：\`${captionText}\`。`);
        return;
    }
...
function checkFormulaRules(blocks, issues) {
    ...
    formulaBlocks.forEach((block) => {
        ...
        if (!labelMatch) {
            addIssue(issues, 'warning', 'formula.no_label', block.line, '公式块缺少编号标注。', '在 $$ 块内任意一行末尾追加 `% 式（X-Y）` 形式的编号，如 `% 式（5-1）`。');
            return;
        }
        ...
        if (chapter !== null && labelChapter !== chapter) {
            addIssue(issues, 'warning', 'formula.chapter_mismatch', block.line, `公式编号 \`${label}\` 的章号为 ${labelChapter}，但当前位置在第 ${chapter} 章。`);
        }
    });
}
function checkReferenceRules(lines, blocks, issues) {
    ...
    if (refDefs.length > 0 && refDefs.length < 20) {
        addIssue(issues, 'warning', 'reference.count_low', 1, `参考文献共 ${refDefs.length} 篇，2026届计算机学院要求不少于 20 篇。`);
    }
    if (refDefs.length > 0) {
        ...
        if (recentRatio < 0.3) {
            addIssue(issues, 'warning', 'reference.recent_ratio_low', 1, `近三年参考文献共 ${recentRefs.length}/${refDefs.length} 篇（${(recentRatio * 100).toFixed(1)}%），2026届计算机学院要求不低于 30%。`);
        }
    }
    ...
    lines.forEach((line, index) => {
        ...
        while ((match = bareRefRe.exec(line)) !== null) {
            addIssue(issues, 'warning', 'reference.bare_citation', index + 1, `疑似裸引用 \`[${match[1]}]\`，官方要求使用上角标方括号格式 \`[^${match[1]}]\`。`, `将 \`[${match[1]}]\` 改为 \`[^${match[1]}]\`。`);
        }
    });
    // 检查参考文献标题是否存在
