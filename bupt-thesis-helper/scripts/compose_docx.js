...
function findDefaultContentType(typesDoc, extension) {
    ...
    return getElementChildren(typesDoc.documentElement).find((node) => (localName(node) === 'Default'
        && String(node.getAttribute('Extension') || '').toLowerCase() === normalized)) || null;
}
function findOverrideContentType(typesDoc, partName) {
    return getElementChildren(typesDoc.documentElement).find((node) => (localName(node) === 'Override'
        && node.getAttribute('PartName') === partName)) || null;
}
...
async function cloneUsedRelationships({ coverZip, bodyZip, coverRelsDoc, bodyRelsDoc, coverTypesDoc, bodyTypesDoc, usedRelationshipIds }) {
    ...
    const existingIds = new Set(getElementChildren(bodyRelationshipsRoot)
...
function mergeMissingStyles(bodyZip, coverZip) {
    ...
    return Promise.all([
        ...
        const existingStyleIds = new Set(getElementChildren(bodyRoot)
...
function findStyleById(stylesDoc, styleId) {
    return getElementChildren(stylesDoc.documentElement).find((node) => (localName(node) === 'style'
        && (node.getAttribute('w:styleId') || node.getAttribute('styleId')) === styleId)) || null;
}
